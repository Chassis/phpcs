# Install PHPCS
#
# @param config Hash containing configuration settings
# @param path Path to the phpcs extension directory
class phpcs (
  Hash $config,
  String $path = '/vagrant/extensions/phpcs',
) {
  ...
  if ( latest == $package ) {
    ...
    exec { 'wordpress cs install':
      command => 'git clone -b main https://github.com/WordPress/WordPress-Coding-Standards.git /vagrant/extensions/phpcs/wpcs && phpcs --config-set installed_paths /vagrant/extensions/phpcs/wpcs',
      path    => ['/bin/', '/sbin/' , '/usr/bin/', '/usr/sbin/'],
